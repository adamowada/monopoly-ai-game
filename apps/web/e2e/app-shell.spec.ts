...
test("opens the app shell and verifies backend health connectivity", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { level: 1, name: "Local Game Research Console" })).toBeVisible();
  await expect(page.getByRole("navigation", { name: "Stack navigation" })).toBeVisible();
